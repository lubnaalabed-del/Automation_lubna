...
test('test', async ({ page }) => {
  ...
  await page.getByTestId('patient-subscription').click();
  await page.getByTestId('pause-subscription-btn').click();
  await page.getByText('ليس لدي القدرة على دفع الاشتراك').click();
  await page.getByRole('button', { name: 'نعم' }).click();
  await page.getByTestId('pause-subscription-btn').click();
  await page.getByText('لست مرتاحاً للأخصائي المعالج أو الطبيب').click();
  await page.getByText('لم تعجبني خدمة العملاء').click();
  await page.getByText('أنهيت رحلة العلاج ولم أعد بحاجة لجلسات العلاج').click();
  await page.getByText('ليس لدي القدرة على دفع الاشتراك').click();
  await page.getByText('غير ذلك').click();
  await page.getByRole('textbox', { name: 'اكتب هنا' }).click();
  await page.getByRole('textbox', { name: 'اكتب هنا' }).fill('Thank you!');
  await page.getByRole('button', { name: 'تأكيد' }).click();
  //Activate subacription
  await page.getByTestId('active-subscription-btn').click();
    //Deactivate AT+
  await page.locator('#at-plus-subscription-toggle').click({force:true});
  await page.waitForTimeout(1000);
  await page.getByRole('button', { name: 'تأكيد' }).click();
  await page.getByRole('link', { name: 'العودة لصفحة اشتراكي' }).click();
});
